...
static_assert(is_list_constructible_v<__int128_t, int>);
static_assert(not is_list_constructible_v<__uint128_t, int>);
static_assert(is_list_constructible_v<__int128_t, unsigned>);
static_assert(is_list_constructible_v<__uint128_t, unsigned>);
static_assert(not is_list_constructible_v<__int128_t, double>);
static_assert(not is_list_constructible_v<__uint128_t, double>);
__DATA_STRUCTURE_END

__DATA_STRUCTURE_START(unit test for ds::is_assignable)
struct unit_test_is_assignable_base {
    int a;
};
struct unit_test_is_assignable_derived : unit_test_is_assignable_base {
    int b;
};
struct unit_test_is_assignable_polymorphic_base {
    int a;
    virtual ~unit_test_is_assignable_polymorphic_base();
};
struct unit_test_is_assignable_polymorphic_derived_1 : unit_test_is_assignable_polymorphic_base {
    int b;
    virtual ~unit_test_is_assignable_polymorphic_derived_1() override;
};
struct unit_test_is_assignable_polymorphic_derived_2 : unit_test_is_assignable_polymorphic_base {
    int c;
};
struct unit_test_is_assignable_polymorphic_derived_3 : unit_test_is_assignable_polymorphic_derived_1, unit_test_is_assignable_polymorphic_derived_2 {
    int d;
};
struct unit_test_is_assignable_polymorphic_virtual_derived_1 : virtual unit_test_is_assignable_polymorphic_derived_1 {
    int b;
};
struct unit_test_is_assignable_polymorphic_virtual_derived_2 : virtual unit_test_is_assignable_polymorphic_derived_1 {
    int c;
};
struct unit_test_is_assignable_polymorphic_virtual_derived_final : unit_test_is_assignable_polymorphic_virtual_derived_1, unit_test_is_assignable_polymorphic_virtual_derived_2 {
    int d;
};
...
struct unit_test_is_assignable_2 {
    unit_test_is_assignable_2() = default;
    unit_test_is_assignable_2(int);
    explicit unit_test_is_assignable_2(char);
    unit_test_is_assignable_2(double) = delete;
    explicit unit_test_is_assignable_2(void *) = delete;
    unit_test_is_assignable_2(int, char);
    explicit unit_test_is_assignable_2(long, long);
    unit_test_is_assignable_2(initializer_list<int *>);
    explicit unit_test_is_assignable_2(initializer_list<char *>);
    unit_test_is_assignable_2(initializer_list<double *>, int, char, long);
    template <typename T, typename U, typename V>
    unit_test_is_assignable_2(T, U &&, V **);
    unit_test_is_assignable_2(void *, void *, void **) = delete;
    operator unsigned short &();
    operator unsigned &() = delete;
    operator const unsigned long &() const &;
    explicit operator float &();
...
static_assert(is_assignable_v<volatile unit_test_is_assignable &&, double ********>);
static_assert(is_assignable_v<const volatile unit_test_is_assignable &&, double ********>);
static_assert(not is_assignable_v<int, int>);
static_assert(not is_assignable_v<int, int &>);
static_assert(not is_assignable_v<int, int &&>);
static_assert(not is_assignable_v<int, const int>);
static_assert(not is_assignable_v<int, volatile int>);
static_assert(not is_assignable_v<int, const volatile int>);
static_assert(not is_assignable_v<int, const int &>);
static_assert(not is_assignable_v<int, volatile int &>);
static_assert(not is_assignable_v<int, const volatile int &>);
static_assert(not is_assignable_v<int, const int &&>);
...
static_assert(is_assignable_v<int &, volatile int>);
static_assert(is_assignable_v<int &, const volatile int>);
static_assert(is_assignable_v<int &, const int &>);
static_assert(is_assignable_v<int &, volatile int &>);
static_assert(is_assignable_v<int &, const volatile int &>);
static_assert(is_assignable_v<int &, const int &&>);
...
static_assert(not is_assignable_v<int &&, volatile int &&>);
static_assert(not is_assignable_v<int &&, const volatile int &&>);
static_assert(not is_assignable_v<const int, int>);
static_assert(not is_assignable_v<const int, int &>);
static_assert(not is_assignable_v<const int, int &&>);
static_assert(not is_assignable_v<const int, const int>);
static_assert(not is_assignable_v<const int, volatile int>);
static_assert(not is_assignable_v<const int, const volatile int>);
static_assert(not is_assignable_v<const int, const int &>);
static_assert(not is_assignable_v<const int, volatile int &>);
static_assert(not is_assignable_v<const int, const volatile int &>);
static_assert(not is_assignable_v<const int, const int &&>);
static_assert(not is_assignable_v<const int, volatile int &&>);
static_assert(not is_assignable_v<const int, const volatile int &&>);
static_assert(not is_assignable_v<volatile int, int>);
static_assert(not is_assignable_v<volatile int, int &>);
static_assert(not is_assignable_v<volatile int, int &&>);
static_assert(not is_assignable_v<volatile int, const int>);
static_assert(not is_assignable_v<volatile int, volatile int>);
static_assert(not is_assignable_v<volatile int, const volatile int>);
static_assert(not is_assignable_v<volatile int, const int &>);
static_assert(not is_assignable_v<volatile int, volatile int &>);
static_assert(not is_assignable_v<volatile int, const volatile int &>);
static_assert(not is_assignable_v<volatile int, const int &&>);
static_assert(not is_assignable_v<volatile int, volatile int &&>);
static_assert(not is_assignable_v<volatile int, const volatile int &&>);
static_assert(not is_assignable_v<const volatile int, int>);
static_assert(not is_assignable_v<const volatile int, int &>);
static_assert(not is_assignable_v<const volatile int, int &&>);
static_assert(not is_assignable_v<const volatile int, const int>);
static_assert(not is_assignable_v<const volatile int, volatile int>);
static_assert(not is_assignable_v<const volatile int, const volatile int>);
static_assert(not is_assignable_v<const volatile int, const int &>);
static_assert(not is_assignable_v<const volatile int, volatile int &>);
static_assert(not is_assignable_v<const volatile int, const volatile int &>);
static_assert(not is_assignable_v<const volatile int, const int &&>);
...
static_assert(not is_assignable_v<const int &, volatile int &&>);
static_assert(not is_assignable_v<const int &, const volatile int &&>);
static_assert(is_assignable_v<volatile int &, int>);
static_assert(is_assignable_v<volatile int &, int &>);
static_assert(is_assignable_v<volatile int &, int &&>);
static_assert(is_assignable_v<volatile int &, const int>);
static_assert(is_assignable_v<volatile int &, volatile int>);
static_assert(is_assignable_v<volatile int &, const volatile int>);
static_assert(is_assignable_v<volatile int &, const int &>);
static_assert(is_assignable_v<volatile int &, volatile int &>);
static_assert(is_assignable_v<volatile int &, const volatile int &>);
static_assert(is_assignable_v<volatile int &, const int &&>);
...
static_assert(not is_assignable_v<const volatile int &&, volatile int &&>);
static_assert(not is_assignable_v<const volatile int &&, const volatile int &&>);
static_assert(not is_assignable_v<int, bool>);
static_assert(not is_assignable_v<int, short>);
static_assert(not is_assignable_v<int, unsigned short>);
static_assert(not is_assignable_v<int, char>);
static_assert(not is_assignable_v<int, unsigned char>);
static_assert(not is_assignable_v<int, signed char>);
static_assert(not is_assignable_v<int, unsigned>);
static_assert(not is_assignable_v<int, long>);
static_assert(not is_assignable_v<int, unsigned long>);
static_assert(not is_assignable_v<int, const unsigned>);
static_assert(not is_assignable_v<int, const long>);
static_assert(not is_assignable_v<int, const unsigned long>);
static_assert(not is_assignable_v<int, char8_t>);
static_assert(not is_assignable_v<int, char16_t>);
static_assert(not is_assignable_v<int, char32_t>);
static_assert(not is_assignable_v<int, wchar_t>);
static_assert(not is_assignable_v<int, float>);
static_assert(not is_assignable_v<double, float>);
static_assert(not is_assignable_v<double, int>);
static_assert(not is_assignable_v<double, unsigned>);
static_assert(not is_assignable_v<double, long double>);
static_assert(not is_assignable_v<double, const int>);
static_assert(not is_assignable_v<double, const unsigned>);
static_assert(is_assignable_v<int &, bool>);
static_assert(is_assignable_v<int &, short>);
static_assert(is_assignable_v<int &, unsigned short>);
static_assert(is_assignable_v<int &, char>);
static_assert(is_assignable_v<int &, unsigned char>);
static_assert(is_assignable_v<int &, signed char>);
static_assert(is_assignable_v<int &, unsigned>);
static_assert(is_assignable_v<int &, long>);
static_assert(is_assignable_v<int &, unsigned long>);
static_assert(is_assignable_v<int &, const unsigned>);
static_assert(is_assignable_v<int &, const long>);
static_assert(is_assignable_v<int &, const unsigned long>);
static_assert(is_assignable_v<int &, char8_t>);
static_assert(is_assignable_v<int &, char16_t>);
static_assert(is_assignable_v<int &, char32_t>);
static_assert(is_assignable_v<int &, wchar_t>);
...
static_assert(is_assignable_v<double &, unsigned>);
static_assert(is_assignable_v<double &, long double>);
static_assert(is_assignable_v<double &, const int>);
static_assert(is_assignable_v<double &, const unsigned>);
static_assert(is_assignable_v<int *&, int []>);
static_assert(is_assignable_v<const int *&, int []>);
static_assert(not is_assignable_v<int *const &, int []>);
static_assert(not is_assignable_v<const int *const &, int []>);
static_assert(is_assignable_v<int *&, int [42]>);
static_assert(is_assignable_v<const int *&, int [42]>);
static_assert(not is_assignable_v<int *const &, int [42]>);
static_assert(not is_assignable_v<const int *const &, int [42]>);
static_assert(not is_assignable_v<int [], initializer_list<int>>);
static_assert(not is_assignable_v<int [42], initializer_list<int>>);
static_assert(not is_assignable_v<int *, initializer_list<int>>);
static_assert(not is_assignable_v<int (&)[], int []>);
static_assert(not is_assignable_v<int (&&)[], int []>);
static_assert(not is_assignable_v<int (&)[42], int [42]>);
static_assert(not is_assignable_v<int (&&)[42], int [42]>);
static_assert(not is_assignable_v<int (&)[], initializer_list<int>>);
static_assert(not is_assignable_v<int (&)[42], initializer_list<int>>);
static_assert(is_assignable_v<int *&, int (&)[]>);
...
static_assert(not is_assignable_v<void (), void ()>);
static_assert(not is_assignable_v<void (&)(), void ()>);
static_assert(not is_assignable_v<void (&&)(), void ()>);
static_assert(not is_assignable_v<void (&)(), void (&&)()>);
static_assert(not is_assignable_v<void (&&)(), void (&)()>);
static_assert(is_assignable_v<void (*&)(), void (&)()>);
static_assert(is_assignable_v<void (*&)(), void (&&)()>);
static_assert(not is_assignable_v<void (*const &)(), void ()>);
static_assert(not is_assignable_v<void (*const &)(), void (&)()>);
static_assert(not is_assignable_v<void (*const &)(), void (&&)()>);
static_assert(not is_assignable_v<int &, C>);
static_assert(is_assignable_v<int &, D>);
static_assert(not is_assignable_v<C &, int>);
static_assert(not is_assignable_v<D &, int>);
static_assert(not is_assignable_v<D &, short>);
static_assert(not is_assignable_v<D &, bool>);
static_assert(not is_assignable_v<D &, unsigned>);
static_assert(not is_assignable_v<D &, unit_test_is_assignable_2>);
static_assert(is_assignable_v<unit_test_is_assignable_base, unit_test_is_assignable_derived>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived, unit_test_is_assignable_base>);
static_assert(is_assignable_v<unit_test_is_assignable_base *&, unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *&, unit_test_is_assignable_base *>);
static_assert(not is_assignable_v<unit_test_is_assignable_base *&, const unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *&, const unit_test_is_assignable_base *>);
static_assert(is_assignable_v<unit_test_is_assignable_base *&, unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *&, unit_test_is_assignable_base *const>);
static_assert(not is_assignable_v<unit_test_is_assignable_base *&, const unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *&, const unit_test_is_assignable_base *const>);
static_assert(is_assignable_v<const unit_test_is_assignable_base *&, unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *&, unit_test_is_assignable_base *>);
static_assert(is_assignable_v<const unit_test_is_assignable_base *&, const unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *&, const unit_test_is_assignable_base *>);
static_assert(is_assignable_v<const unit_test_is_assignable_base *&, unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *&, unit_test_is_assignable_base *const>);
static_assert(is_assignable_v<const unit_test_is_assignable_base *&, const unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *&, const unit_test_is_assignable_base *const>);
static_assert(not is_assignable_v<unit_test_is_assignable_base *const &, unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *const &, unit_test_is_assignable_base *>);
static_assert(not is_assignable_v<unit_test_is_assignable_base *const &, const unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *const &, const unit_test_is_assignable_base *>);
static_assert(not is_assignable_v<unit_test_is_assignable_base *const &, unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *const &, unit_test_is_assignable_base *const>);
static_assert(not is_assignable_v<unit_test_is_assignable_base *const &, const unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived *const &, const unit_test_is_assignable_base *const>);
static_assert(not is_assignable_v<const unit_test_is_assignable_base *const &, unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *const &, unit_test_is_assignable_base *>);
static_assert(not is_assignable_v<const unit_test_is_assignable_base *const &, const unit_test_is_assignable_derived *>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *const &, const unit_test_is_assignable_base *>);
static_assert(not is_assignable_v<const unit_test_is_assignable_base *const &, unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *const &, unit_test_is_assignable_base *const>);
static_assert(not is_assignable_v<const unit_test_is_assignable_base *const &, const unit_test_is_assignable_derived *const>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived *const &, const unit_test_is_assignable_base *const>);
static_assert(is_assignable_v<unit_test_is_assignable_base &, unit_test_is_assignable_derived &>);
static_assert(is_assignable_v<unit_test_is_assignable_base &, unit_test_is_assignable_derived &&>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived &, unit_test_is_assignable_base &>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived &, unit_test_is_assignable_base &&>);
static_assert(is_assignable_v<unit_test_is_assignable_base &, const unit_test_is_assignable_derived &>);
static_assert(is_assignable_v<unit_test_is_assignable_base &, const unit_test_is_assignable_derived &&>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived &, const unit_test_is_assignable_base &>);
static_assert(not is_assignable_v<unit_test_is_assignable_derived &, const unit_test_is_assignable_base &&>);
static_assert(not is_assignable_v<const unit_test_is_assignable_base &, unit_test_is_assignable_derived &>);
static_assert(not is_assignable_v<const unit_test_is_assignable_base &, unit_test_is_assignable_derived &&>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived &, unit_test_is_assignable_base &>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived &, unit_test_is_assignable_base &&>);
static_assert(not is_assignable_v<const unit_test_is_assignable_base &, const unit_test_is_assignable_derived &>);
...
static_assert(not is_assignable_v<const unit_test_is_assignable_derived &&, const unit_test_is_assignable_base &>);
static_assert(not is_assignable_v<const unit_test_is_assignable_derived &&, const unit_test_is_assignable_base &&>);
static_assert(is_assignable_v<unit_test_is_assignable_polymorphic_base, unit_test_is_assignable_polymorphic_derived_1>);
static_assert(is_assignable_v<unit_test_is_assignable_polymorphic_base, unit_test_is_assignable_polymorphic_derived_2>);
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_base, unit_test_is_assignable_polymorphic_derived_3>);
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_derived_1, unit_test_is_assignable_polymorphic_base>);
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_derived_2, unit_test_is_assignable_polymorphic_base>);
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_derived_3, unit_test_is_assignable_polymorphic_base>);
static_assert(is_assignable_v<unit_test_is_assignable_polymorphic_base *&, unit_test_is_assignable_polymorphic_derived_1 *>);
static_assert(is_assignable_v<unit_test_is_assignable_polymorphic_base *&, unit_test_is_assignable_polymorphic_derived_2 *>);
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_base *&, unit_test_is_assignable_polymorphic_derived_3 *>);
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_derived_1 *&, unit_test_is_assignable_polymorphic_base *>);
...
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_derived_2 &, unit_test_is_assignable_polymorphic_base &>);
static_assert(not is_assignable_v<unit_test_is_assignable_polymorphic_derived_3 &, unit_test_is_assignable_polymorphic_base &>);
static_assert(is_assignable_v<unit_test_is_assignable_2 &>);
static_assert(is_assignable_v<unit_test_is_assignable_2 &, int>);
static_assert(is_assignable_v<unit_test_is_assignable_2 &, char>);
static_assert(not is_assignable_v<unit_test_is_assignable_2 &, double>);
static_assert(not is_assignable_v<unit_test_is_assignable_2 &, void *>);
static_assert(not is_assignable_v<unit_test_is_assignable_2 &, long>);
static_assert(not is_assignable_v<unit_test_is_assignable_2 &, int *>);
static_assert(is_assignable_v<unsigned short &, unit_test_is_assignable_2>);
static_assert(is_assignable_v<unsigned short &, unit_test_is_assignable_2>);
static_assert(not is_assignable_v<unsigned &, unit_test_is_assignable_2>);
static_assert(not is_assignable_v<unsigned long &, unit_test_is_assignable_2>);
static_assert(not is_assignable_v<unsigned long &, unit_test_is_assignable_2 &>);
static_assert(is_assignable_v<unsigned long &, const unit_test_is_assignable_2 &>);
static_assert(not is_assignable_v<float &, unit_test_is_assignable_2>);
static_assert(not is_assignable_v<double &, unit_test_is_assignable_2>);
static_assert(is_assignable_v<long double &, const unit_test_is_assignable_2 &>);
static_assert(not is_assignable_v<int **&, unit_test_is_assignable_2>);        // prefer to operator int **&()
static_assert(not is_assignable_v<int **&&, unit_test_is_assignable_2>);      // still prefer to operator int **&()
static_assert(is_assignable_v<__int128_t &, int>);
static_assert(is_assignable_v<__uint128_t &, int>);
static_assert(is_assignable_v<__int128_t &, unsigned>);
static_assert(is_assignable_v<__uint128_t &, unsigned>);
static_assert(is_assignable_v<__int128_t &, double>);
static_assert(is_assignable_v<__uint128_t &, double>);
__DATA_STRUCTURE_END
...
struct unit_test_is_convertible_base {
    int a;
};
struct unit_test_is_convertible_derived : unit_test_is_convertible_base {
    int b;
};
struct unit_test_is_convertible_polymorphic_base {
    int a;
    virtual ~unit_test_is_convertible_polymorphic_base();
};
struct unit_test_is_convertible_polymorphic_derived_1 : unit_test_is_convertible_polymorphic_base {
    int b;
    virtual ~unit_test_is_convertible_polymorphic_derived_1() override;
};
struct unit_test_is_convertible_polymorphic_derived_2 : unit_test_is_convertible_polymorphic_base {
    int c;
};
struct unit_test_is_convertible_polymorphic_derived_3 : unit_test_is_convertible_polymorphic_derived_1, unit_test_is_convertible_polymorphic_derived_2 {
    int d;
};
struct unit_test_is_convertible_polymorphic_virtual_derived_1 : virtual unit_test_is_convertible_polymorphic_derived_1 {
    int b;
};
struct unit_test_is_convertible_polymorphic_virtual_derived_2 : virtual unit_test_is_convertible_polymorphic_derived_1 {
    int c;
};
struct unit_test_is_convertible_polymorphic_virtual_derived_final : unit_test_is_convertible_polymorphic_virtual_derived_1, unit_test_is_convertible_polymorphic_virtual_derived_2 {
    int d;
};
struct unit_test_is_convertible {
    unit_test_is_convertible() = default;
    unit_test_is_convertible(int);
    explicit unit_test_is_convertible(char);
    unit_test_is_convertible(double) = delete;
    explicit unit_test_is_convertible(void *) = delete;
    unit_test_is_convertible(int, char);
    explicit unit_test_is_convertible(long, long);
    unit_test_is_convertible(initializer_list<int *>);
    explicit unit_test_is_convertible(initializer_list<char *>);
    unit_test_is_convertible(initializer_list<double *>, int, char, long);
    template <typename T, typename U, typename V>
    unit_test_is_convertible(T, U &&, V **);
    unit_test_is_convertible(void *, void *, void **) = delete;
    operator unsigned short &();
    operator unsigned &() = delete;
    operator const unsigned long &() const &;
    explicit operator float &();
...
static_assert(is_convertible_v<int, const int &>);
static_assert(not is_convertible_v<int, volatile int &>);
static_assert(not is_convertible_v<int, const volatile int &>);
static_assert(is_convertible_v<int, const int &&>);
static_assert(is_convertible_v<int, volatile int &&>);
static_assert(is_convertible_v<int, const volatile int &&>);
static_assert(not is_convertible_v<int, int *>);
...
static_assert(is_convertible_v<int &, volatile int &>);
static_assert(is_convertible_v<int &, const volatile int &>);
static_assert(not is_convertible_v<int &, const int &&>);
static_assert(not is_convertible_v<int &, volatile int &&>);
static_assert(not is_convertible_v<int &, const volatile int &&>);
static_assert(is_convertible_v<int &&, int>);
static_assert(not is_convertible_v<int &&, int &>);
...
static_assert(not is_convertible_v<const int, int &>);
static_assert(not is_convertible_v<const int, int &&>);
...
static_assert(is_convertible_v<const int, const int &>);
static_assert(not is_convertible_v<const int, volatile int &>);
static_assert(not is_convertible_v<const int, const volatile int &>);
static_assert(is_convertible_v<const int, const int &&>);
static_assert(not is_convertible_v<const int, volatile int &&>);
static_assert(is_convertible_v<const int, const volatile int &&>);
static_assert(is_convertible_v<volatile int, int>);
static_assert(not is_convertible_v<volatile int, int &>);
static_assert(not is_convertible_v<volatile int, int &&>);
...
static_assert(not is_convertible_v<volatile int, const int &>);
static_assert(not is_convertible_v<volatile int, volatile int &>);
static_assert(not is_convertible_v<volatile int, const volatile int &>);
static_assert(not is_convertible_v<volatile int, const int &&>);
static_assert(is_convertible_v<volatile int, volatile int &&>);
static_assert(is_convertible_v<volatile int, const volatile int &&>);
static_assert(is_convertible_v<const volatile int, int>);
static_assert(not is_convertible_v<const volatile int, int &>);
static_assert(not is_convertible_v<const volatile int, int &&>);
...
static_assert(is_convertible_v<const volatile int, const volatile int>);
static_assert(not is_convertible_v<const volatile int, const int &>);
static_assert(not is_convertible_v<const volatile int, volatile int &>);
static_assert(not is_convertible_v<const volatile int, const volatile int &>);
static_assert(not is_convertible_v<const volatile int, const int &&>);
static_assert(not is_convertible_v<const volatile int, volatile int &&>);
static_assert(is_convertible_v<const volatile int, const volatile int &&>);
static_assert(is_convertible_v<const int &, int>);
...
static_assert(not is_convertible_v<const int &, volatile int &>);
static_assert(is_convertible_v<const int &, const volatile int &>);
static_assert(not is_convertible_v<const int &, const int &&>);
static_assert(not is_convertible_v<const int &, volatile int &&>);
static_assert(not is_convertible_v<const int &, const volatile int &&>);
static_assert(is_convertible_v<volatile int &, int>);
static_assert(not is_convertible_v<volatile int &, int &>);
...
static_assert(is_convertible_v<int, short>);
static_assert(is_convertible_v<int, unsigned short>);
static_assert(is_convertible_v<int, char>);
static_assert(is_convertible_v<int, unsigned char>);
static_assert(is_convertible_v<int, signed char>);
static_assert(is_convertible_v<int, unsigned>);
static_assert(is_convertible_v<int, long>);
static_assert(is_convertible_v<int, unsigned long>);
static_assert(is_convertible_v<int, const unsigned>);
static_assert(is_convertible_v<int, const long>);
static_assert(is_convertible_v<int, const unsigned long>);
static_assert(is_convertible_v<int, char8_t>);
static_assert(is_convertible_v<int, char16_t>);
static_assert(is_convertible_v<int, char32_t>);
static_assert(is_convertible_v<int, wchar_t>);
static_assert(is_convertible_v<int, float>);
static_assert(is_convertible_v<double, float>);
static_assert(is_convertible_v<double, int>);
static_assert(is_convertible_v<double, unsigned>);
static_assert(is_convertible_v<double, long double>);
static_assert(is_convertible_v<double, const int>);
static_assert(is_convertible_v<double, const unsigned>);
static_assert(not is_convertible_v<int *, int []>);
static_assert(not is_convertible_v<const int *, int []>);
static_assert(not is_convertible_v<int *const, int []>);
static_assert(not is_convertible_v<const int *const, int []>);
static_assert(not is_convertible_v<int *, int [42]>);
static_assert(not is_convertible_v<const int *, int [42]>);
static_assert(not is_convertible_v<int *const, int [42]>);
static_assert(not is_convertible_v<const int *const, int [42]>);
static_assert(not is_convertible_v<int [], initializer_list<int>>);
static_assert(not is_convertible_v<int [42], initializer_list<int>>);
static_assert(not is_convertible_v<int *, initializer_list<int>>);
static_assert(not is_convertible_v<int (&)[], int []>);
static_assert(not is_convertible_v<int (&&)[], int []>);
static_assert(not is_convertible_v<int (&)[42], int [42]>);
static_assert(not is_convertible_v<int (&&)[42], int [42]>);
static_assert(not is_convertible_v<int (&)[], initializer_list<int>>);
static_assert(not is_convertible_v<int (&)[42], initializer_list<int>>);
static_assert(not is_convertible_v<int *, int (&)[]>);
...
static_assert(is_convertible_v<D, int>);
static_assert(is_convertible_v<D, short>);
static_assert(is_convertible_v<D, bool>);
static_assert(is_convertible_v<D, unsigned>);
static_assert(is_convertible_v<D, unit_test_is_list_constructible>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base, unit_test_is_list_constructible_derived>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived, unit_test_is_list_constructible_base>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *, unit_test_is_list_constructible_derived *>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *, unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *, const unit_test_is_list_constructible_derived *>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *, const unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *, unit_test_is_list_constructible_derived *const>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *, unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *, const unit_test_is_list_constructible_derived *const>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *, const unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *, unit_test_is_list_constructible_derived *>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_derived *, unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *, const unit_test_is_list_constructible_derived *>);
static_assert(is_convertible_v<const unit_test_is_list_constructible_derived *, const unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *, unit_test_is_list_constructible_derived *const>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_derived *, unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *, const unit_test_is_list_constructible_derived *const>);
static_assert(is_convertible_v<const unit_test_is_list_constructible_derived *, const unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *const, unit_test_is_list_constructible_derived *>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *const, unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *const, const unit_test_is_list_constructible_derived *>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *const, const unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *const, unit_test_is_list_constructible_derived *const>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *const, unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base *const, const unit_test_is_list_constructible_derived *const>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived *const, const unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *const, unit_test_is_list_constructible_derived *>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_derived *const, unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *const, const unit_test_is_list_constructible_derived *>);
static_assert(is_convertible_v<const unit_test_is_list_constructible_derived *const, const unit_test_is_list_constructible_base *>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *const, unit_test_is_list_constructible_derived *const>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_derived *const, unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base *const, const unit_test_is_list_constructible_derived *const>);
static_assert(is_convertible_v<const unit_test_is_list_constructible_derived *const, const unit_test_is_list_constructible_base *const>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base &, unit_test_is_list_constructible_derived &>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base &, unit_test_is_list_constructible_derived &&>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived &, unit_test_is_list_constructible_base &>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_derived &, unit_test_is_list_constructible_base &&>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base &, const unit_test_is_list_constructible_derived &>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_base &, const unit_test_is_list_constructible_derived &&>);
static_assert(is_convertible_v<unit_test_is_list_constructible_derived &, const unit_test_is_list_constructible_base &>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_derived &, const unit_test_is_list_constructible_base &&>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base &, unit_test_is_list_constructible_derived &>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_base &, unit_test_is_list_constructible_derived &&>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_derived &, unit_test_is_list_constructible_base &>);
static_assert(not is_convertible_v<const unit_test_is_list_constructible_derived &, unit_test_is_list_constructible_base &&>);
...
static_assert(is_convertible_v<const unit_test_is_list_constructible_derived &&, const unit_test_is_list_constructible_base &>);
static_assert(is_convertible_v<const unit_test_is_list_constructible_derived &&, const unit_test_is_list_constructible_base &&>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_base, unit_test_is_list_constructible_polymorphic_derived_1>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_base, unit_test_is_list_constructible_polymorphic_derived_2>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_base, unit_test_is_list_constructible_polymorphic_derived_3>);
static_assert(is_convertible_v<unit_test_is_list_constructible_polymorphic_derived_1, unit_test_is_list_constructible_polymorphic_base>);
static_assert(is_convertible_v<unit_test_is_list_constructible_polymorphic_derived_2, unit_test_is_list_constructible_polymorphic_base>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_derived_3, unit_test_is_list_constructible_polymorphic_base>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_base *, unit_test_is_list_constructible_polymorphic_derived_1 *>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_base *, unit_test_is_list_constructible_polymorphic_derived_2 *>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_base *, unit_test_is_list_constructible_polymorphic_derived_3 *>);
static_assert(is_convertible_v<unit_test_is_list_constructible_polymorphic_derived_1 *, unit_test_is_list_constructible_polymorphic_base *>);
...
static_assert(is_convertible_v<unit_test_is_list_constructible_polymorphic_derived_2 &, unit_test_is_list_constructible_polymorphic_base &>);
static_assert(not is_convertible_v<unit_test_is_list_constructible_polymorphic_derived_3 &, unit_test_is_list_constructible_polymorphic_base &>);
static_assert(is_convertible_v<unit_test_is_list_constructible, int>);
static_assert(not is_convertible_v<unit_test_is_list_constructible, char>);
static_assert(not is_convertible_v<unit_test_is_list_constructible, double>);
static_assert(not is_convertible_v<unit_test_is_list_constructible, void *>);
static_assert(not is_convertible_v<unit_test_is_list_constructible, long>);
static_assert(not is_convertible_v<unit_test_is_list_constructible, int *>);
static_assert(is_convertible_v<unsigned short, unit_test_is_list_constructible>);
static_assert(is_convertible_v<unsigned short &, unit_test_is_list_constructible>);
static_assert(not is_convertible_v<unsigned, unit_test_is_list_constructible>);
static_assert(not is_convertible_v<unsigned long, unit_test_is_list_constructible>);
static_assert(not is_convertible_v<unsigned long, unit_test_is_list_constructible &>);
static_assert(not is_convertible_v<unsigned long, const unit_test_is_list_constructible &>);
static_assert(not is_convertible_v<float, unit_test_is_list_constructible>);
static_assert(not is_convertible_v<double, unit_test_is_list_constructible>);
